...
(function (SAT) {
    ...
    function sat(pos1, pos2, p1, p2, iscircle1, iscircle2) {
        if (iscircle1 === void 0) { iscircle1 = false; }
        if (iscircle2 === void 0) { iscircle2 = false; }
        if (p1.length < 1 || p2.length < 1)
            return false;
        var ismuti1 = (p1[0].constructor.name == "Array");
        var ismuti2 = (p2[0].constructor.name == "Array");
        if (!ismuti1 && !ismuti2) {
        ...
        if (ismuti1 && ismuti2) {
            for (i = 0; i < p1.length; i++) {
                for (j = 0; j < p1.length; j++) {
                    if (convexsat(pos1, pos2, p1[i], p2[j], iscircle1, iscircle2))
                        return true;
                }
            }
            return false;
        ...
        if (ismuti1) {
            for (i = 0; i < p1.length; i++) {
                if (convexsat(pos1, pos2, p1[i], p2, iscircle1, iscircle2))
                    return true;
            }
            return false;
        }
    ...
    function convexsat(pos1, pos2, p1, p2, iscircle1, iscircle2) {
        if (iscircle1 === void 0) { iscircle1 = false; }
        if (iscircle2 === void 0) { iscircle2 = false; }
        ...
        if (iscircle1 && iscircle2)
            return circleHit(pos1, pos2, p1, p2);
        var szaxis = [];
        if (iscircle1 && iscircle2) {
            var segment = new egret.Point(pos1.x - pos2.x, pos1.y - pos2.y);
    ...
    //圆形与圆形碰撞检测
    function circleHit(pos1, pos2, p1, p2) {
    ...
    SAT.circleHit = circleHit;
    //获得多边形包围盒(相对值).顺序:左上右下四个值→Xmin,Ymin,Xmax,Ymax
    function getPolyBound(p) {
        if (p.length < 1)
            return [0, 0, 0, 0];
        var szpt = [p[0].x, p[0].y, p[0].x, p[0].y];
    ...
    SAT.getPolyBound = getPolyBound;
    //获得圆形包围盒(相对值)(参数:两项,第一项为圆心坐标,第二项x,y都为半径,后面也许会扩充椭圆).顺序:左上右下四个值→Xmin,Ymin,Xmax,Ymax
    function getCircleBound(p) {
        if (p.length != 2)
            return [0, 0, 0, 0];
        var szpt = [p[0].x - p[1].x, p[0].y - p[1].x, p[0].x + p[1].x, p[0].y + p[1].x];
        return szpt;
    }
    SAT.getCircleBound = getCircleBound;
    //包围盒碰撞检测.顺序:左上右下四个值→Xmin,Ymin,Xmax,Ymax
    function boundHit(pos1, pos2, p1, p2) {
        if (p1.length != 4 || p2.length != 4)
            return false;
        if (!overlap(new egret.Point(p1[0] + pos1.x, p1[2] + pos1.x), new egret.Point(p2[0] + pos2.x, p2[2] + pos2.x)))
            return false;
        if (!overlap(new egret.Point(p1[1] + pos1.y, p1[3] + pos1.y), new egret.Point(p2[1] + pos2.y, p2[3] + pos2.y)))
            return false;
        return true;
    }
    SAT.boundHit = boundHit;
    //点乘
    function dot(v1, v2) {
    ...
    //获取多边形在投影轴上最小与最大点
    function getProjection(pos, p, axis) {
    ...
    //获取圆形在投影轴上最小与最大点
    function getCircleProjection(pos, p, axis) {
        var ret = new egret.Point;
